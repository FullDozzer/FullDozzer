...
def _build_entry(block, subgroup: str | None = None) -> dict:
    subject = _extract_subject(block) if block is not None else ""
    room = _extract_room(block) if block is not None else ""
    teacher = _extract_teacher(block) if block is not None else ""
    has_pair = any([subject, room, teacher])
    ...
    for pair in data["pairs"]:
        ...
        if len(entries) > 1:
            for idx, entry in enumerate(entries):
                ...
                if not entry.get("has_pair", True):
                    out.append("📖 Нет пары")
                    continue
...
def parse_schedule_data(html: str) -> dict:
    ...
    for card in cards:
        ...
        has_subgroup_layout = subgroup1 is not None or subgroup2 is not None

        entries: list[dict] = []

        if has_subgroup_layout:
            entries.append(_build_entry(subgroup1, subgroup="1"))
            entries.append(_build_entry(subgroup2, subgroup="2"))
        else:
...
def render_schedule_image(schedule_data: dict, output_path: str) -> str:
    ...
    for pair in schedule_data["pairs"]:
        ...
        for entry in pair["entries"]:
            if not entry.get("has_pair", True):
                card_height += line_height(body_font) + block_spacing
                continue
    ...
    for pair in schedule_data["pairs"]:
        ...
        for idx, entry in enumerate(pair["entries"]):
            ...
            if not entry.get("has_pair", True):
                draw.text((card_left + 40, content_y), "Нет пары", font=body_font, fill=text_muted)
